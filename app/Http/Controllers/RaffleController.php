...
class RaffleController extends Controller
{
    ...
    public function publicWinners()
    {
        ...
        return view('public-winners', compact('winners'));
    }
    ...
    public function updateCoupon(Request $request, Coupon $coupon)
    {
        ...
        return back()->with('success', 'Kupon berhasil diupdate.')->with('active_page', 'coupons');
    }
...
{
    $winnerBuyerNames = Winner::with('coupon')
        ->get()
        ->pluck('coupon.buyer_name')
        ->filter()
        ->unique()
        ->values()
        ->toArray();
    ...
    $coupon = Coupon::where('payment_status', 'PAID')
        ...
        ->when(count($winnerBuyerNames) > 0, function ($query) use ($winnerBuyerNames) {
            $query->whereNotIn('buyer_name', $winnerBuyerNames);
        })
        ->inRandomOrder()
        ->first();
    ...
    if (!$coupon) {
        return response()->json([
            'success' => false,
            'message' => 'Tidak ada kupon lunas yang tersedia untuk diundi atau semua pembeli lunas sudah pernah menang.',
        ], 422);
    }
